...
def dir_subpackages(tmp_path_factory, dir_orig_data) -> pathlib.Path:
    ...
    for file in files:
        ...
        shutil.copy2(str(file_source), str(temp_dir))
    return temp_dir
...
def dir_lib_x_flac(tmp_path_factory, dir_orig_data):
    ...
    shutil.copytree(s_path, temp_dir, dirs_exist_ok=True)
    return temp_dir
...
def dir_lib_c_ogg(tmp_path_factory, dir_orig_data):
    ...
    temp_dir = tmp_path_factory.mktemp("C_ogg", numbered=True)
